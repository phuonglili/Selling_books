...
<div class="btn-group" style="margin-left: 120px; width:200px;">
  ...
  <ul class="dropdown-menu">
	...
	<form method="post">
     <p style="margin-top:100px;font-size:30px;text-align:center;"><b>SẢN PHẨM</b></p>
    <div class="container">
        <div class="row">
       <?php
...
else 
{
  ...
  $sql = "SELECT * FROM  sach WHERE tensach LIKE BINARY '%$searchbook%'  ORDER BY masach DESC";
    ...
    while($row = $result->fetch_assoc()) 
      {
        ...
        {
          $tensach = str_replace($searchbook,"<span style='color:red'>${searchbook}</span>",$row['tensach']);
          $img = $row['image'];
          $gia = str_replace($searchbook,"<span style='color:red'>${searchbook}</span>",$row['giatien']);
          $masach = $row['masach'];
        }
        echo '<div class="col-md-2 col-6">
        <div style = "background-color:#f7f7f7;border-radius:5px;border:1px solid white;">
         <a href = "trangmuasach.php?id='.$masach.'">
          <img src= '.$img.' style ="width:100%;margin-top:20px;height:200px;"></a>
          <p style = "font-size:20px;"><b>'.$tensach.'</b></p>
          <p style = "font-size:20px;"><b>'.$gia.'VNĐ</b></p>
        </div></div> ';
      }
  } 
    else 
